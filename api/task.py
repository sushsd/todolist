    ...
    def get(self):
        ...
        if logged_in_user:
            ...
            if user:
                ...
                tasks_pagination= Task.query.filter_by(user_id=user.id).paginate(page=page, per_page=per_page, error_out=False)
                task_list = [
                    {
                        'id':task.id,
    ...
    def post(self):
        ...
        if logged_in_user:
            ...
            if user:
                ...
                new_task = Task(user_id = user.id,task_title=json_data['newTaskTitle'],description=json_data['newTaskDescription'])
    ...
    def delete(self):
        ...
        if logged_in_user:
            ...
            if user:
                json_data = request.get_json()
                task_id = json_data.get('id')
                task = UserLogins.query.filter_by(id=task_id,user_id= user.id).first()
